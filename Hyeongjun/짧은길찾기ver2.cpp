...
void list::add(edge road) {
	head->next->prev = &nodes[node_idx];
	nodes[node_idx].road = road;
	nodes[node_idx].prev = head;
	nodes[node_idx].next = head->next;
	head->next = &nodes[node_idx++];
}

void list::del(node* del_node) {
...
void init() {
	for (int i = 0; i < MAX_N; i++) {
		HEAD[i].next = &TAIL[i];
		HEAD[i].prev = 0;
		HEAD[i].road = edge{0,0};
		TAIL[i].next = 0;
		TAIL[i].prev = &HEAD[i];
		TAIL[i].road = edge{ 0,0 };
...
void dijikstra() {
	...
	while (!is_empty()) {
		...
		node* nxt = adj[cur.idx].head->next;
		while (nxt != &TAIL[cur.idx]) {
